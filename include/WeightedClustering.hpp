#ifndef AI_TOOLBOX_UTILS_WEIGHTED_CLUSTERING_HEADER_FILE
#define AI_TOOLBOX_UTILS_WEIGHTED_CLUSTERING_HEADER_FILE
...
namespace AIToolbox {
    template <typename Gen, typename M2D, typename V>
    std::pair<std::vector<unsigned>, Matrix2D> weightedKMeans(size_t clusters, const Eigen::MatrixBase<M2D> & data, const Eigen::MatrixBase<V> & weights, Gen &) {
        ...
        // We use k-means++ to initialize the centers.
        // Select first cluster uniformly.

        std::vector<size_t> chosenCenters(clusters);
        Vector dists(data.rows());

        if (clusters != 2) throw 5;

        // Initialize from empirical means of the two sets of input data.
        means.row(0) = data.topRows(data.rows()/2).transpose() * (weights.topRows(data.rows()/2));
        means.row(1) = data.bottomRows(data.rows()/2).transpose() * (weights.topRows(data.rows()/2));

        // std::cout << "### Running KMEANS (" << clusters << " clusters) ###" << '\n';
        // std::cout << "Assigned points: " << counts.transpose() << '\n';
        // std::cout << "Initial clusters initialized:\n" << means << '\n';
        ...
        for (unsigned i = 0; i < maxIterations; ++i) {
            ...
            for (auto p = 0; p < data.rows(); ++p) {
                ...
                means.row(c) += data.row(p) * weights[p];
                counts[c] += weights[p];
            }
            // Normalize them
            means.array().colwise() /= counts.array().cwiseMax(1.0); // cwiseMax to prevent division by zero

            // std::cout << "New assignments:\n"; for (auto a : assignments) std::cout << a << ' '; std::cout << '\n';
            // std::cout << "Assigned points: " << counts.transpose() << '\n';
            // std::cout << "New clusters computed:\n" << means << '\n';
        }
    ...
    template <typename Gen, typename M2D, typename V>
    GMM weightedEM(size_t clusters, const Eigen::MatrixBase<M2D> & data, const Eigen::MatrixBase<V> & weights, Gen & gen) {
        // The changes to the standard EM algorithm are shown in
        // the paper
        //
        // "EM Algorithms for Weighted-Data Clustering with
        // Application to Audio-Visual Scene Analysis"
        //
        // in section III.
        constexpr double covarianceDiag = 1e-6;

        auto [assignments, means] = weightedKMeans(clusters, data, weights, gen);
        ...
        //
        // Note that, differently from the code in the while loop, here the
        // covariance is computed in a "simple" way, as the responsibilities
        // for now simply represent the weights of each point.
        //
        // Thus, in this case, the denominator (pi) needs to be equal to the
        // sum of all per-cluster weights.
        ...
        while (true) {
            ...
            //
            // When computing the weighted version of Normal(mean, cov), it is equivalent to
            // raising each Normal to the power of w. However, the same (proportionally equivalent)
            // effect can be generated by simply computing Normal(mean, 1/w * cov).
            //
            // Since we are working with the precision here, the equivalent
            // precision is simply (w * prec) (see Equations 6, 1, 2).
            for (size_t c = 0; c < clusters; ++c) {
                covHelper = (data * precs[c]).rowwise() - means.row(c) * precs[c];
                // We don't need to modify the determinant as it will only change up to a constant
                responsibilities.row(c) = precs[c].diagonal().prod() * (-0.5 * covHelper.array().square().rowwise().sum().transpose() * weights.array().transpose()).exp();
            }
            responsibilities.array().colwise() *= pi.array();

            // Compute new log-logLikelihood
            ...
            // Now the rest. Turns out pi is unaffected by the weights (see
            // Equation 8).
            pi = responsibilities.rowwise().sum(); // N_k
            pi = pi.cwiseMax(10.0 * std::numeric_limits<double>::min()); // Prevent division by zero

            // Note that here we are not actually changing the
            // responsibilities. But anywhere else we need them, we need them
            // weighted, so we simply do it once here.
            responsibilities.array().rowwise() *= weights.array().transpose();

            // The means use the weighted responsibilities (see Equation 9).
            means = responsibilities * data;
            means.array().colwise() /= responsibilities.array().rowwise().sum().cwiseMax(10.0 * std::numeric_limits<double>::min()); // #######################

            // Finally, the covariances, where the numerator is weighted, but
            // the denominator is not (see Equation 10).
            for (size_t c = 0; c < clusters; ++c) {
                // N x D matrix
                covHelper = data.rowwise() - means.row(c);
                // D x D matrix
            ...
            // Finally, normalize the cluster weights.
            pi /= pi.sum();
        }
    ...
    template <typename Gen, typename M2D, typename V>
    GMM variationalWeightedEM(size_t clusters, const Eigen::MatrixBase<M2D> & data, const Eigen::MatrixBase<V> & weights, Gen & gen, double alpha0 = 0.0, double beta0 = 0.0) {
        Matrix2D covHelper(data.rows(), data.cols());
        ...
        while (true) {
            ...
            //
            // The changes performed here mirror the weightedEM() function, so
            // please look there. I could not find another source specific to
            // variational inference, so hopefully my changes here make sense.
            //
            // In brief, Nk should stay the same, but means & covariances
            // should be weighted.
            ...
            responsibilities.array().rowwise() *= weights.array().transpose(); // #############################
            means = responsibilities * data; // ###############################333
            means.array().colwise() /= responsibilities.array().rowwise().sum().cwiseMax(10.0 * std::numeric_limits<double>::min()); // #######################
            ...
            for (size_t c = 0; c < clusters; ++c) {
                ...
                covHelper = (data * precs[c]).rowwise() - means.row(c) * precs[c];
                responsibilities.row(c) = -0.5 * covHelper.array().square().rowwise().sum().transpose() * weights.array().transpose();
            }
            ...
            // std::cout << '[' << iter << "] Optimizing... " << std::abs(lowerBound - prevLowerBound) << '\n';
    ...
    template <typename Gen, typename M2D, typename V>
    GMM weightedKMeansToNormals(size_t clusters, const Eigen::MatrixBase<M2D> & data, const Eigen::MatrixBase<V> & weights, Gen & gen) {
        // This is basically weightedEM without the EM part..
        ...
        // std::cout << "Cluster 0 total weight PI: " << pi[0] << "; responsibilities: " << responsibilities.row(0).sum() << '\n';
        ...
        // for (int p = 0; p < data.rows(); ++p) {
        //     if (assignments[p] == 0) {
        //         std::cout << data.row(p) << " with weight " << weights[p] << '\n';
        //     }
        // }
        // std::cout << "Mean:\n" << means.row(0) << '\n';
        // std::cout << "Covariance:\n" << covs[0] << '\n';
        // std::cout << "Total weights: " << weights.sum() << '\n';
